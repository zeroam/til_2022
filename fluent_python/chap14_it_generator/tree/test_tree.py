...
def test_1_level():
    ...
    expected = [("One", 0, True)]
    result = list((cls.__name__, level, last) for cls, level, last in tree(One))
...
def test_2_levels_2_leaves():
    ...
    result = list((cls.__name__, level, last) for cls, level, last in tree(Branch))
    assert expected == result
...
def test_3_levels_1_leaf():
    ...
    result = list((cls.__name__, level, last) for cls, level, last in tree(X))
    assert expected == result
...
def test_4_levels_1_leaf():
    ...
    result = list((cls.__name__, level, last) for cls, level, last in tree(Level0))
    assert expected == result
...
def test_4_levels_4_leaves():
    ...
    result = list(tree(A))
    result = list((cls.__name__, level, last) for cls, level, last in tree(A))
    assert expected == result
...
def test_many_levels_1_leaf():
    ...
    result = list((cls.__name__, level, last) for cls, level, last in tree(Root))
